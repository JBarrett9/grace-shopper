...
import { addToppingToPizza, destroyPizzaToppings, fetchPizza, updatePizzaById } from "../../../api";
...
const Edit = ({ sizes, crusts, user, handleUpdate, toppings }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    let userId = user.id
    await updatePizzaById(localStorage.getItem("token"), id, { name, crustId, sizeId, userId, featured })
    await destroyPizzaToppings(id,localStorage.getItem("token"))

    selectedOption.forEach(async (el) => {
      let r = await addToppingToPizza(localStorage.getItem("token"), id, el.value.id, amount, double)
      console.log(r)
    })
